...
class Modaration extends CI_Controller {
    public $year;
    ...
    {   
        $data['_title']             = "Modaration Files";
        $data['files']              = $this->general_model->get_files_for_index('4',$this->session->userdata('year'));
        $this->load->template('modaration/file',$data);
    }

    public function edit_file($id = false){
        if($id){
            if($this->general_model->_get_file_byid($id)){
                $data['_title']             = "Edit File - Modaration";
                $data['file']               = $this->general_model->_get_file_byid($id)[0];
                $this->load->template('modaration/edit_file',$data);
            }
            else
            {
                $this->session->set_flashdata('error', 'Fie Not Found');
                redirect(base_url().'modaration');
            }
        }
        else
        {
    ...
    public function update_file()
    {
        ...
        else
        {
            ...
            redirect(base_url().'modaration/edit_file/'.$this->input->post('file_id'));
        }
    }
    ...
    public function add_file_view()
    {
        ...
        $data['_title']             = "Add New File - Modaration";
        $this->load->template('modaration/add_filee',$data);
    }

    public function add_file()
    {

        if($this->general_model->get_files_new('4',$this->input->post('sem')))
        {
            $no = count($this->general_model->get_files_new('4',$this->input->post('sem'))) + 1;
        }
        else{
            $no = 1;
        }
        ...
        $data = [
                        'no'            =>  $no,
                        'head'          =>  '4',
                        'file_name'     =>  'modaration_file'.$no.$sem,
                        'title'         =>  $this->input->post('title'),
                        'message'       =>  $this->input->post('message'),
        ...
        if($this->db->insert('file', $data)){
            ...
            $query = 'CREATE TABLE IF NOT EXISTS `modaration_file'.$no.$sem.'` (
    ...
    public function add_data($id = false)
    {
        ...
        if($id){
            if($this->general_model->get_original_file($id,'4'))
            {

                $data['file']           = $this->general_model->get_original_file($id,'4')[0];
                $data['_id']            = $id;
                $data['_title']         = 'Modaration - File-'.$data['file']['no'];
                ...
                if($data['old_data_num'] > 0){
                    $this->load->template('modaration/edit_data',$data);
                }else{
                    $this->load->template('modaration/add_data',$data);
                }
            }
            else{
    ...
    public function view_data($id = false)
    {
        ...
        if($id){
            if($this->general_model->get_original_file($id,'4'))
            {
                ...
                if($data['old_data_num'] > 0){
                    $this->load->template('modaration/view',$data);
                }else{
    ...
    public function view_data_print($id = false)
    {
        if($id){
            if($this->general_model->get_original_file($id,'4'))
            {
                ...
                if($data['old_data_num'] > 0){
                    $this->load->view('modaration/view_user_print',$data);
                }else{
                    $this->session->set_flashdata('error', 'No Data Found');
                    redirect(base_url().'modaration');
                }
            ...
            if($this->general_model->get_original_file($id,'4'))
            {
                ...
                if($dis_acc > 0){
                    ...
                    $spreadsheet->getProperties()->setCreator('BKNMU - Powered By - Kava Developers')
                        ...
                        ->setDescription('Modaration - '.'File-'.$data['file']['no'])
                    ...
                    $filename = 'Modaration Bank Copy-File-'.$data['file']['no'].'('.$year_active.') '.date('d-M-y h i A');
        ...
        if($id){
            ...
            if($this->general_model->get_original_file($id,'4'))
            {
                ...
                if($dis_acc > 0){
                    ...
                    $filename = 'Modaration Corporation Bank Copy-File-'.$data['file']['no'].'('.$year_active.') '.date('d-M-y h i A');
    ...
    public function print_bank($id = false)
    {
        ...
        if($id){
            if($this->general_model->get_original_file($id,'4'))
            {
                ...
                $data['file']           = $this->general_model->get_original_file($id,'4')[0];
                $this->load->view('modaration/bank_print',$data);
    ...
    public function view_bills($id = false)
    {
        ...
        if($id){
            if($this->general_model->get_original_file($id,'4'))
            {
                ...
                $data['file']           = $this->general_model->get_original_file($id,'4')[0];
                $this->load->template('modaration/view_bills',$data);
    ...
    public function print_corp_bank($id = false)
    {
        ...
        if($id){
            if($this->general_model->get_original_file($id,'4'))
            {
                ...
                $data['_title']             = "Print Bank Copy";
                $data['file']           = $this->general_model->get_original_file($id,'4')[0];
                $this->load->view('modaration/corp_bank_print',$data);
    ...
    public function view_all_bank()
    {
        ...
        $data['files']              = $this->general_model->get_all_files('4',$this->session->userdata('year'));
        $this->load->template('modaration/view_all_bills',$data);
    }
        ...
        if($data['files'])
        {
            ...
            if($aa > 0){
                ...
                $spreadsheet->getProperties()->setCreator('BKNMU - Powered By - Kava Developers')
                    ...
                    ->setDescription('Modaration - '.$this->session->userdata('year'))
                    ->setCategory('Bank Copy');
                ...
                foreach ($data['files'] as $key => $file_row) {
                    ...
                    $filename = 'Modaration Bank Copy-('.$year_active.') '.date('d-M-y h i A');
        ...
        if($data['files'])
        {
            ...
            if($aa > 0){
                ...
                foreach ($data['files'] as $key => $file_row) {
                    ...
                    $filename = 'Modaration Corporation Bank Copy-('.$year_active.') '.date('d-M-y h i A');
