...
class Key_persons extends CI_Controller {
	...
	{	
		$data['page_title']	= 'Manage Key Persons';
		$data['key_persons'] = $this->users->key_persons();
		$this->load->template('key_persons/index',$data);
	}


	public function add()
	{
	    $data['page_title']	= 'Add Key Persons';
		$this->load->template('key_persons/add',$data);
	}
	...
	public function view($id = false)
	{
		...
		if($id)
		{
			...
			{
				$data['page_title']	= 'View Key Persons';
			    $data['user'] = $this->users->get_user($id)[0];
				$data['detail'] = $this->users->business_detail($id)[0];
				$this->load->template('key_persons/view',$data);
			}	
			else
			{
				$this->session->set_flashdata('error', 'Key Persons Not Found');
	        	redirect(base_url().'key_persons');
			}
	...
	public function save(){
		...
		******************************************************/
		...
		else
		{
			...
			$user_insert = array(
		        'user_type'           => 	'business',
				'user_type_id'        => 	$this->my_custom_func_model->get_key_person_id('business'),
			...
			if($this->db->insert('user', $user_insert)){
				...
				if($this->db->insert('business_partners', $busines_insert))
				{
					...
					if(!empty($_FILES['my_image']['name']))
					{
						...
						if($this->upload->do_upload('my_image'))
						{
							...
							{
								$this->session->set_flashdata('msg', 'Key Persons Successfully Added');
	        					redirect(base_url().'key_persons');
							}
							else
							{
								$this->session->set_flashdata('error', 'Key Persons Successfully Added - '.$this->upload->display_errors());
				...
				{
					$this->session->set_flashdata('error', 'Problem In Add Key Persons Try Again');
	        		redirect(base_url().'key_persons/add');
				}
			}
			else
			{
	...
	public function edit($id = false){
		if($id)
		{
			if($this->users->get_user_with_type($id,'business')[0])
			{
				$data['page_title']	= 'Edit Key Persons';
				$data['user'] = $this->users->get_key_persons($id)[0];
				$data['business'] = $this->users->business_detail($id)[0];
				$this->load->template('key_persons/edit',$data);
			}	
			else
			{
				$this->session->set_flashdata('error', 'Keu Persons Not Found');
	...
	public function update(){
		...
		******************************************************/
		...
		{
			$data['page_title']	= 'Edit Key Persons';
			$data['user'] = $this->users->get_key_persons($this->input->post('main_id'))[0];
		...
		else
		{
			...
			if($this->db->update('user', $user_insert)){
				...
				if($this->db->update('business_partners', $agent_insert))
				{
					if(!empty($_FILES['my_image']['name']))
					{
						...
						if($this->upload->do_upload('my_image'))
						{
							...
							if($this->db->update('user', $subadmin_insert))
							{
								$this->session->set_flashdata('msg', 'Keu Persons Successfully Saved');
	        					redirect(base_url().'key_persons');
							}
							else
							{
								$this->session->set_flashdata('error', 'Keu Persons Successfully Saved - '.$this->upload->display_errors());
								unlink('./uploads/'.$key_personse);
	        					redirect(base_url().'key_persons');
							}
						}
						else
				...
				{
					$this->session->set_flashdata('error', 'Problem In Save Key Persons Try Again');
	        		redirect(base_url().'key_persons');
				}
			}
			else
		...
		if($id)
		{
			if($this->users->get_user_with_type($id,'business')[0])
			{
				...
				if($this->db->update('user',array('updated_by'  => 	$this->session->userdata('id'),'delete_flag' => '1','deleted_at' => date('Y-m-d H:i:s'))))
				{
					...
					$this->session->set_flashdata('msg', 'Key Persons Successfully Deleted');
	        		redirect(base_url().'key_persons');
				}
				else{
					$this->session->set_flashdata('error', 'Key Persons Not Deleted Try Again');
	        		redirect(base_url().'key_persons');
				}
			}
			else{
